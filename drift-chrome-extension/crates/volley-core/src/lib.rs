//! volley-core — the real-time control/DSP plane of Volley.
//!
//! Volley's architecture is **one audio bus, two engines**:
//!   * JavaScript owns the *model plane* (Whisper / Qwen / Kokoro ONNX on WebGPU).
//!   * This crate owns the *control plane* — the deterministic, audio-rate logic:
...
//! (unit-tested here, and reusable by a terminal twin) and to
//! `wasm32-unknown-unknown` for the browser worker. The wasm-bindgen surface is
//! in [`wasm`], gated to the wasm target so native builds never touch JS glue.
...
pub struct DuplexFsm {
    ...
    /// where Thinking lasts ~100 ms. A slow REMOTE brain (e.g. the Claude CLI bridge, 3–7 s to
    /// first token) holds the FSM in Thinking long enough that ordinary mic energy — the user's
    /// own trailing speech, breathing, room noise — repeatedly fires a Thinking-barge and aborts
    /// the turn before the reply lands, so no reply ever commits. The worker sets this `false`
    /// for such brains; Speaking-phase barge-in is unaffected (you can still cut the agent off
    /// once it's talking).
...
impl DuplexFsm {
    ...
    pub fn reset(&mut self) {
        ...
        self.output_level = 0.0; // VC-10: reset is now fully idempotent (parity with set_speaking(false))
    }
...
mod tests {
    ...
    fn barge_hangover_boundary_tolerate_then_reset() {
        // Ported from the deleted standalone Vad test onto the FSM's INLINED hangover (detect_barge):
        // exactly `vad_hang` quiet frames mid-barge are tolerated (run preserved), the (vad_hang+1)th
        // resets it. Pins the boundary the dead Vad struct used to guard.
        let cfg = Config::default();
        let mut fsm = DuplexFsm::new(cfg.clone());
        fsm.set_speaking(true);
        let loud = frame(0.3);
        let quiet = frame(0.0);
        // Build some barge progress (< barge_speech_frames so no BargeIn yet).
        for _ in 0..(cfg.barge_speech_frames - 1) {
            assert_eq!(fsm.on_mic_frame(&loud), Action::None);
        }
        let run_before = fsm.barge_run();
        assert!(run_before > 0);
        // Exactly vad_hang quiet frames are tolerated — the run is preserved (not reset).
        ...
        // One MORE quiet frame (vad_hang+1 total) crosses the hangover → run resets.
        assert_eq!(fsm.on_mic_frame(&quiet), Action::None);
        assert_eq!(
            fsm.barge_run(),
            0,
    ...
    fn commit_reason_survives_set_thinking() {
        // VC-A1: the end-silence commit reason (last_reason=2) must survive the worker's immediate
        // set_thinking() so JS can observe WHY a turn committed. (set_speaking/reset still clear it.)
        let cfg = Config::default();
        let mut fsm = DuplexFsm::new(cfg.clone());
        let loud = frame(0.2);
        let quiet = frame(0.0);
        ...
        fsm.set_thinking(); // worker calls this right after COMMIT
        assert_eq!(
            fsm.last_reason(),
            2,
            "commit reason must survive set_thinking (observable by JS)"
        );
        // But a turn boundary (reset / set_speaking(false)) clears it.
        fsm.reset();
    ...
    fn utterance_commits_after_end_silence() {
        ...
        // First loud frame → UserStarted.
        assert_eq!(fsm.on_mic_frame(&loud), Action::UserStarted);
        assert_eq!(fsm.state(), State::Listening);
        // Enough speech to clear min_speech_frames.
        for _ in 0..10 {
            assert_eq!(fsm.on_mic_frame(&loud), Action::None);
        }
        // Trailing silence (must exceed vad_hang so VAD goes inactive, then
        // end_silence_frames of counted silence) eventually commits.
        let mut committed = false;
        for _ in 0..60 {
            if fsm.on_mic_frame(&quiet) == Action::CommitUtterance {
                committed = true;
    ...
    fn barge_in_continues_as_one_utterance_without_restart() {
        // CONTRACT the worker relies on: on returning BargeIn the FSM is ALREADY mid-utterance
        // (state=Listening, speech_frames=1, counting this frame as the start). Continued speech must
        // therefore return None — NOT another UserStarted — so the worker keeps the pre-roll-seeded
        // capture buffer instead of overwriting it. (This is why the worker must NOT call
        // set_speaking(false) on barge-in: that would reset speech_frames and re-fire UserStarted.)
        let mut fsm = DuplexFsm::new(Config::default());
        fsm.set_speaking(true);
        let loud = frame(0.3);
        let mut barged = false;
        ...
        assert!(barged, "sustained speech should barge in");
        assert_eq!(fsm.state(), State::Listening);
        // Continued speech: mid-utterance, so every frame is None (never a second UserStarted).
        for _ in 0..10 {
            assert_eq!(fsm.on_mic_frame(&loud), Action::None);
        }
        // And the barge-in utterance still end-points and commits after trailing silence.
        let quiet = frame(0.0);
        let mut committed = false;
        for _ in 0..80 {
            if fsm.on_mic_frame(&quiet) == Action::CommitUtterance {
        ...
        assert!(
            committed,
            "barge-in utterance should commit after end silence"
        );
        assert_eq!(fsm.state(), State::Thinking);
    }
    ...
    fn barge_in_tolerates_brief_dips() {
        // Real speech over the agent dips between syllables. A short sub-gate dip must NOT reset the
        // barge counter — otherwise the user has to sustain perfectly-consecutive loud frames to
        // interrupt (slow/flaky, and the audio takes too long to stop). With single-frame dips
        // (≤ vad_hang), `barge_speech_frames` loud frames still trigger a barge-in.
        let cfg = Config::default();
        let mut fsm = DuplexFsm::new(cfg.clone());
        fsm.set_speaking(true);
        let loud = frame(0.3);
    ...
    fn barge_run_resets_after_long_gap() {
        // A gap longer than vad_hang means it wasn't sustained speech: the run resets, so a stray
        // loud blip can't later combine with new speech into a premature barge-in.
        let cfg = Config::default();
        let mut fsm = DuplexFsm::new(cfg.clone());
        fsm.set_speaking(true);
        let loud = frame(0.3);
        let quiet = frame(0.0);
        // A few loud frames (< barge_speech_frames), then a long quiet gap → run resets.
        for _ in 0..(cfg.barge_speech_frames - 1) {
            assert_eq!(fsm.on_mic_frame(&loud), Action::None);
        }
        ...
        // Post-reset it takes the FULL count again: barge_speech_frames-1 are None, then BargeIn.
        for _ in 0..(cfg.barge_speech_frames - 1) {
            assert_eq!(fsm.on_mic_frame(&loud), Action::None);
        }
    ...
    fn thinking_phase_barge_enters_listening_with_one_utterance() {
        // Full-duplex during Thinking (ASR+brain): a sustained user burst before the agent speaks is a
        // barge-in that supersedes the in-flight turn. Mirrors barge_in_continues_as_one_utterance_without_restart
        // but starting from Thinking; gate is the plain vad_threshold (nothing is playing yet).
        let cfg = Config::default();
        let mut fsm = DuplexFsm::new(cfg.clone());
        fsm.set_thinking();
        assert_eq!(fsm.state(), State::Thinking);
        ...
        assert_eq!(fsm.state(), State::Listening);
        // Continued speech is ONE utterance (no second UserStarted), then commits after end-silence.
        for _ in 0..10 {
            assert_eq!(fsm.on_mic_frame(&loud), Action::None);
        }
        let quiet = frame(0.0);
    ...
    fn thinking_barge_disabled_never_supersedes() {
        // With barge_in_thinking=false (the worker sets this for slow remote brains), NO amount of
        // sustained speech during Thinking may barge — the long think-wait must be allowed to land.
        // The FSM stays in Thinking the whole time. Speaking-phase barge is exercised elsewhere and
        ...
        // Re-enabling restores full-duplex behavior (sanity: the flag is the only thing gating it).
    ...
    fn thinking_brief_noise_does_not_barge() {
        // A short noise burst during Thinking (< barge_speech_frames) must NOT barge — the user has
        // to actually start talking again. Keeps the think window from triggering on trailing blips.
        let cfg = Config::default();
        let mut fsm = DuplexFsm::new(cfg.clone());
        fsm.set_thinking();
        let loud = frame(0.2);
    ...
    fn zero_crossing_rate_basics() {
        // Constant frame → no sign changes → ZCR 0; full sign-alternation → ZCR ~1; empty → 0.
    ...
    fn reset_clears_output_level() {
        // reset() must clear output_level (parity with set_speaking(false)), else a fresh conversation
        // inherits a stale, elevated echo gate. Verified via the Speaking-phase gate.
        let mut fsm = DuplexFsm::new(Config::default());
        fsm.set_speaking(true);
        fsm.set_output_level(0.5); // would raise the Speaking gate to 0.015 + 0.6*0.5 = 0.315
        fsm.reset();
        // Re-enter Speaking WITHOUT setting an output level: the gate must be the base 0.015, so a
        // moderate 0.1 burst barges in — it would NOT if the stale 0.315 gate had survived reset().
        fsm.set_speaking(true);
        let loud = frame(0.1);
        let mut barged = false;
        for _ in 0..Config::default().barge_speech_frames {
    ...
    fn adaptive_floor_raises_onset_threshold_in_noise() {
        // A SUB-threshold ambient bed lifts the tracked floor, so a louder transient that WOULD clear
        // the fixed 0.015 gate no longer false-fires an onset. With the floor OFF the same transient does.
        let mut cfg = Config::default();
        cfg.floor_margin = 4.0; // adaptive ON
        let mut fsm = DuplexFsm::new(cfg);
        let bed = frame(0.012); // BELOW 0.015 → never onsets, feeds the noise floor EMA
        let spike = frame(0.04); // ABOVE 0.015 (would onset at the fixed gate)
        for _ in 0..300 {
            assert_eq!(fsm.on_mic_frame(&bed), Action::None); // bed never onsets; floor climbs
        }
        // Floor converged ≈0.012 → gate ≈ 0.015 + 4*0.012 ≈ 0.063, above the 0.04 transient.
        assert!(
            fsm.effective_gate() > 0.04,
            "adaptive floor should lift the gate above the transient"
        );
        assert_eq!(fsm.on_mic_frame(&spike), Action::None); // transient rejected (no false onset)
        assert_eq!(fsm.state(), State::Listening);

        // Control: floor OFF → the same transient fires an onset at the fixed 0.015 gate.
    ...
    fn brief_blip_does_not_commit() {
        ...
        let loud = frame(0.2);
        let quiet = frame(0.0);
        // 2 loud frames < min_speech_frames(5) → just a blip.
        assert_eq!(fsm.on_mic_frame(&loud), Action::UserStarted);
        fsm.on_mic_frame(&loud);
        for _ in 0..40 {
    ...
    fn echo_does_not_self_interrupt_but_user_can() {
        let mut fsm = DuplexFsm::new(Config::default());
        fsm.set_speaking(true);
        fsm.set_output_level(0.2); // agent is loud; gate = 0.015 + 0.6*0.2 = 0.135
                                   // Echo bleeding into the mic at 0.1 (< gate) must NOT barge in.
        for _ in 0..20 {
            assert_eq!(fsm.on_mic_frame(&frame(0.1)), Action::None);
        }
        assert_eq!(fsm.state(), State::Speaking);
        // The user talking clearly over the agent at 0.3 (> gate) DOES barge in.
        let mut barged = false;
        for _ in 0..Config::default().barge_speech_frames {
    ...
    fn resample_interpolates_and_scales_length() {
        ...
        let down = resample_linear(&x, 2, 1); // 2× downsample → len 2, picks 0 and 2
        assert_eq!(down.len(), 2);
        assert!((down[0] - 0.0).abs() < 1e-6);
        assert!((down[1] - 2.0).abs() < 1e-6);
        assert_eq!(resample_linear(&x, 16000, 16000), x.to_vec()); // identity
        assert_eq!(resample_linear(&x, 1, 2).len(), 8); // 2× upsample
