...
| **Engagement** | 104 conversation comments · 100 inline review comments |

## Why this PR is notable

Adding `node --run` to execute package.json scripts. The standout is `MylesBorins` asking the *prior* question — *'the project needs to seriously consider if it wants to take on this functionality and the benefit it provides'* — while `wraithgar` and `fabiospampinato` weigh a 200ms speedup against added surface area.

## 🧠 The lesson for reviewers

> The highest-altitude review question isn't 'is the code correct?' — it's **'should this feature exist here at all?'** Weigh marginal benefit against perpetual maintenance and user-facing complexity.

## How the author framed it (PR description excerpt)

> > This is a more up to date implementation of @joyeecheung's original [pull-request](https://github.com/nodejs/node/pull/46534).
> 
...
> - [x] Add documentation
> - [x] Add tests
> - [x] Add `node_modules/.bin` to PATH.
> - [x] Pass additional arguments to `execSync`
> 
> ## Benchmark
> 
> ```
...
> 
> npm can not generally take advantage of lazy loading because it has to be able to install over the top of itself.  Part of installation is the removal of the old code, and if a `require` statement happens after that code removal, npm is now unrecoverably broken.  In fact there are [some versions of npm 7 that are …​ *[truncated]*
...
> 
> As seen in #46534 the default way to run a script in the node+npm stack, `npm run`, is roughly 10x slower than necessary, and `npm run` is a very common command to execute, I wouldn't be surprised if worldwide it gets executed hundreds of millions of times a day, can we imagine if every common tool that we as developers use got 1 …​ *[truncated]*
